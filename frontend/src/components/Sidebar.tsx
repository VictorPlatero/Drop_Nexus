...
export default function Sidebar({ active, onNavigate }: { active: DashboardSection; onNavigate(section: DashboardSection): void }) {
  ...
  return <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col border-r border-line bg-panel md:flex">
    <div className="border-b border-line px-6 py-6">
      <div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-button bg-accent"><Repeat2 size={19} /></div>
        <div><div className="font-semibold text-white">Database Nexus</div><div className="text-xs text-zinc-500">Replication Intelligence</div></div>
      </div>
    </div>
    <nav className="flex-1 space-y-1 p-3">
      {items.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => { onNavigate(id); navigate("/dashboard"); }}
        className={`flex w-full items-center gap-3 rounded-button px-3 py-2.5 text-left text-sm ${active === id ? "bg-blue-600/15 text-blue-400" : "text-zinc-400 hover:bg-zinc-900 hover:text-zinc-200"}`}>
        <Icon size={18} />{label}
      </button>)}
